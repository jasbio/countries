...
export class DropDownModel {
  ...
  queryServer = async (searchText: string) => {
    ...
    try {
      const { data, status } = await axios.get<CountryProp[]>(
        searchText === ''
          ? 'https://jasbio.herokuapp.com/countries?fields=name/all?fields=;name;flag;alpha3Code;'
          : `https://jasbio.herokuapp.com/countries?query=${searchText}&fields=name/all?fields=;name;flag;alpha3Code;`,
        { validateStatus: false }
      )
      // console.log(data, status)
      this.filteredList = status === 200 ? data : []
